...
# You must provide your own Project64 payload (see instructions).
...
class PayloadOverlay:
    ...
    def _prepare_payload(self):
        ...
        try:
            ...
            if not source_found:
                logger.warning("No payload source found. The launcher will likely fail.")
...
class EmuAIPro:
    def __init__(self, root: tk.Tk):
        ...
        # Initialize persist_var BEFORE any UI creation
        ...
        # Check for payload
        if not (self.overlay.pj64_dir / "Project64.exe").exists():
    ...
    def _build_legacy_menus(self):
        ...
        system_menu.add_command(label="Save State", command=lambda: self._log("Legacy save stubbed"))
        system_menu.add_command(label="Load", command=lambda: self._log("Legacy load stubbed"))
    ...
    def _pulse(self):
        ...
        if self.emu_process and self.emu_process.poll() is not None:
    ...
    def _about(self):
        ...
        messagebox.showinfo(
            APP_TITLE, 
            "EmuAI Pro - Legacy Wrapper\n\nA portable frontend for Project64 1.6."
        )
    ...
    def apply_plugin(self):
        """Apply the selected graphics plugin with improved error handling."""
    ...
    def start_emu(self):
        """Start the emulator with improved error handling and process management."""
        if self.running:
    ...
    def stop_emu(self):
        """Stop the emulator process with improved error handling."""
        if self.emu_process:
            self._log("Stopping emulator process...")
            try:
    ...
    def reset_emu(self):
        """Stop and restart the emulator with the same ROM."""
        if not self.running:
